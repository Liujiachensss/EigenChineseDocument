...
void BasicLinearSolving()
{
        ...
        //在此示例中，colPivHouseholderQr()方法返回类 ColPivHouseholderQR 的对象。
        //由于此处的矩阵类型为Matrix3f，因此该行可能已替换为：
        //ColPivHouseholderQR<Matrix3f> dec(A);
        //Vector3f x = dec.solve(b);
        //在这里，ColPivHouseholderQR 是具有选择列主元功能的QR分解。
        ...
        // LDLT	                         ldlt()	                         Positive or negative semidefinite半正定	        +++                 +            ++
        // BDCSVD	                         bdcSvd()	                         None	                                           -                  -	          +++
        // JacobiSVD	                    jacobiSvd()	                    None	                                           -	                 ---	     +++
        //要了解不同分解的真实相对速度，请查看此基准测试。

        {
                //所有这些分解都提供了一个solve()方法，该方法与上述示例一样。
                //如果您对矩阵的性质有更多的了解，您可以使用上表来选择最佳方法。
                //例如，对于求解具有满秩非对称矩阵的线性系统，一个很好的选择是PartialPivLU。
                //如果您知道您的矩阵也是对称且正定的，那么上表表明LLT或LDLT分解是一个非常好的选择。
                //这里有一个例子，也证明了使用一般矩阵（不是向量）作为右手边是可能的：
                Matrix2f A, b;
                A << 2, -1, -1, 3;
                b << 1, 2, 3, 1;
                cout << "Here is the matrix A:\n"
...
void LeastSquaresSolving() // 最小二乘求解

{
        ...
        // 与SVD相比，另一种通常更快、同样准确的方法是完全正交分解（Complete Orthogonal Decomposition）。
        // 同样，如果您对这个问题了解得更多，上面的表包含了可能更快的方法。如果您的矩阵是全秩的，HouseHolderQR是选择的方法。
        // 如果您的矩阵是满秩且条件良好的，则在普通方程的矩阵上使用Cholesky分解（LLT）可以更快。我们关于最小二乘求解的页面有更多的细节
}

void Checking_if_a_matrix_is_singular(){
     LOG();
     // 只有你自己知道你想允许解决方案的错误范围是多少，才能将其视为有效的。因此，Eigen允许您自行进行此类计算，例如下面这个例子：
...
void Computing_eigenvalues_and_eigenvectors(){
     LOG();
     // 这里需要进行特征分解，请查看该页面上(Catalogue of dense decompositions) 可用的分解方法。请确保检查您的矩阵是否为自共轭矩阵，这在类似问题中经常出现。
     // 这里使用 `SelfAdjointEigenSolver` 进行了一个示例，它可以很容易地通过使用 `EigenSolver` 或 `ComplexEigenSolver` 来适应一般的矩阵。
     // 计算特征值和特征向量的过程并不一定收敛，但这种无法收敛的情况非常罕见。调用 info() 是为了检查是否存在这种情况。
     Eigen::Matrix2f A;
     A << 1, 2, 2, 3;
...
void SeparatingTheComputationFromTheConstruction()
{
        ...
        //最后，您可以告诉分解构造函数预先分配存储空间以分解给定大小的矩阵，以便在随后分解此类矩阵时，不执行动态内存分配
        //(当然，如果您使用的是固定大小的矩阵，则不存在动态内存分配完全发生)。
...
void RankRevealingDecompositions()
{
        ...
        //Eigen选择一个明智的默认阈值，该阈值取决于分解，但通常是主对角线大小乘以机器误差ε。
