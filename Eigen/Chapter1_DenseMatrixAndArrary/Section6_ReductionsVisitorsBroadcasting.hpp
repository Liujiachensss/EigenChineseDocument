...
{
        LOG();
        //在Eigen中，约简是一个采用矩阵或数组并返回单个标量值的函数。最常用的归约方法之一是.sum()，它返回给定矩阵或数组内所有元素的总和。
        Eigen::Matrix2d mat;
        mat << 1, 2,
               3, 4;
...
{
        LOG();
        // 2范数的平方可以通过squaredNorm()获得。它本身等于矢量的点积，并且等效于其元素的平方绝对值的总和。
        ...
        // 如果需要其他按元素分配的l^p范数，请使用lpNorm <p>()。
        // 如果需要无穷范数，则模板参数p可以采用特殊值Infinity，这是元素绝对值的最大值。
        VectorXf v(2);
        ...
        m << 1, -2,
            -3, 4;
        // 向量范数
        cout << "v.squaredNorm() = " << v.squaredNorm() << endl; // 2范数的平方
        cout << "v.norm() = " << v.norm() << endl; // 2范数
        cout << "v.lpNorm<1>() = " << v.lpNorm<1>() << endl; // 1范数
        cout << "v.lpNorm<Infinity>() = " << v.lpNorm<Infinity>() << endl; // 无穷范数
        cout << endl;
        // 矩阵范数
        cout << "m.squaredNorm() = " << m.squaredNorm() << endl; // Frobenius范数的平方
        cout << "m.norm() = " << m.norm() << endl; // Frobenius范数
        cout << "m.lpNorm<1>() = " << m.lpNorm<1>() << endl; // 1范数
...
void Visitors()
{
        LOG();
        // 这是在矩阵和数组的所有元素中
        //想要获得一个元素在Matrix或Array中的位置时，访问者很有用。
        //最简单的示例是maxCoeff（＆x，＆y）和minCoeff（＆x，＆y），可用于查找Matrix或Array中最大或最小元素的位置。
...
void PartialReductions()
{
        ...
        // column-wise按列操作、返回行向量，row-wise按行操作、返回列向量。
        Eigen::MatrixXf mat(2, 4);
        mat << 1, 2, 6, 9,
               3, 1, 7, 2;

        std::cout << "Column's maximum: " << std::endl
                  << mat.colwise().maxCoeff() << std::endl; // 对于矩阵mat的每一列，取最大元素值

        // 也可以对行操作
        std::cout << "Row's maximum: " << std::endl
                  << mat.rowwise().maxCoeff() << std::endl; // 对于矩阵mat的每一行，取最大元素值
}
...
void CombiningPartialReductionsWithOtherOperations()
{
        ...
        float maxNorm = mat.colwise().sum().maxCoeff(&maxIndex); //  对于矩阵的每一列中的元素求和，结果的最大元素在第2列
...
void Broadcasting()
{
        ...
        //用两种等效的方式解释指令mat.colwise() += v:
        //1 它将向量添加v到矩阵的每一列。
        //2 可以将其解释为重复向量v四次以形成4*2的矩阵，然后将其按元素加到mat
        mat.colwise() += v;
        ...
        // 在matrix类上，我们可以执行-=,+=,+,-操作，但是不能进行*=,/=,*,/操作
        // 在array类上我们可执行*=,/=,*,/操作
        // If you want multiply column 0 of a matrix mat with v(0), column 1 with v(1), and so on, then use 
        {        
            Eigen::VectorXf v(4);
            v << 1, 2, -1, -2;
        ...
        // 如果不满足，则会出现编译时错误。
        // 这也意味着，在使用Matrix进行这些操作时，广播操作只能应用于Vector类型的对象。
        // 这同样适用于数组类VectorXf是ArrayXf。与往常一样，您不应在同一表达式中混合使用数组和矩阵。
