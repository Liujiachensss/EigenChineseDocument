...
// 此外，Array类提供了一种执行逐元素运算的简便方法，该运算可能没有线性代数含义，
//! 例如将常数加到数组中的每个元素或两个数组按元素相乘。
...
//! 注意二维数组与矩阵的区别：矩阵默认是按列存储的，而数组是按行存储的。
// Type	                                     Typedef
...
void AdditionAndSubtraction()
{
        // 加减法
        // 两个数组的加减法与矩阵相同。
        // 如果两个数组的大小相同，并且该加法或减法是按元素进行的，则此操作有效。
        // 数组还支持以下形式的表达式，该表达式array + scalar将标量加到数组中的每个元素上。
        //! 这提供了不能直接用于Matrix对象的功能。(但是Matrix乘除以标量是可以的！)
        LOG();
        ArrayXXf a(3, 3);
        ArrayXXf b(3, 3);
        a << 1, 2, 3,
...
{
        // 数组类定义除上述加法，减法和乘法运算符其他元素为单位的运算。
        // 例如，.abs()方法获取每个元素的绝对值，而.sqrt()计算元素的平方根。
        //如果您有两个大小相同的数组，则可以调用.min()来构造其元素是两个给定数组中对应元素的最小值的数组。
...
void ConvertingBetweenArrayAndMatrixExpressions()
{
        ...
        // 矩阵表达式具有.array()方法，可以将它们“转换”为数组表达式，因此可以轻松地应用按元素进行运算。
        ...
        // 例如，语句需要两个矩阵和，他们两个转换为阵列，用来将它们相乘元素明智并将结果指定给矩阵变量
        //（这是合法的，因为本征允许分配数组表达式到矩阵的变量）。result = m.array() * n.array()mnresult

        // 实际上，这种使用情况非常普遍，以至于Eigen为矩阵提供了const .cwiseProduct()方法来计算元素乘积。
        LOG();
        MatrixXf m(2, 2);
        MatrixXf n(2, 2);
        MatrixXf result(2, 2);
